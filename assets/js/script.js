...
(function ($) {
  ...
  \****************************************/
				...
				/***/ function (module, __webpack_exports__, __webpack_require__) {
					...
					var _win,
						...
						_renderPropWithEnd = function _renderPropWithEnd(
							ratio,
							data
						) {
							return data.set(
								...
								ratio === 1
									? data.e
									: Math.round((data.s + data.c * ratio) * 10000) /
											10000 +
											data.u,
								data
							);
						...
						_setterScale = function _setterScale(
							target,
							property,
							value
						) {
						...
						_initCore = function _initCore() {
							if (_windowExists() && window.document) {
								...
								_transformOriginProp =
									_checkPropPrefix(_transformOriginProp);
						...
						_convertToUnit = function _convertToUnit(
							...
							var curValue = parseFloat(value) || 0,
								curUnit =
									(value + "").trim().substr((curValue + "").length) ||
									"px",
							...
							if (
								...
								horizontal &&
								cache.time ===
									_gsap_core_js__WEBPACK_IMPORTED_MODULE_0__["_ticker"]
										.time
							) {
								return Object(
									_gsap_core_js__WEBPACK_IMPORTED_MODULE_0__["_round"]
						...
						_tweenComplexCSSString = function _tweenComplexCSSString(
							...
							if (endValues.length) {
								...
								pt.c =
									index < end.length
										? end.substring(index, end.length)
  ...
  */
						...
						_parseTransform = function _parseTransform(target, uncache) {
							...
							if (matrix !== _identity2DMatrix) {
								...
								if (matrix.length === 6) {
									...
									skewX =
										c || d ? _atan2(c, d) * _RAD2DEG + rotation : 0;
									...
									skewX =
										Math.abs(angle) > 0.0002 ? angle * _RAD2DEG : 0;
							...
							if (
								(cache.zOrigin = parseFloat(origin.split(" ")[2]) || 0)
							) {
						...
						_addPxTranslate = function _addPxTranslate(
							target,
							start,
							value
						) {
									...
									if (isTransformRelated) {
										if (!transformPropTween) {
											...
											cache.renderTransform ||
												_parseTransform(target); // if, for example, gsap.set(... {transform:"translateX(50vw)"}), the _get() call doesn't parse the transform, thus cache.renderTransform won't be set yet so force the parsing of the transform here.
										...
										if (p === "scale") {
											...
											endValue =
												_convertKeywordsToPercentages(endValue); //in case something like "left top" or "bottom right" is passed in. Convert to percentages.
											...
											if (cache.svg) {
												...
												endUnit =
													parseFloat(endValue.split(" ")[2]) || 0; //handle the zOrigin separately!
  ...
  \****************************************/
				...
				/***/ function (module, __webpack_exports__, __webpack_require__) {
					...
					var _config = {
						...
						_arrayContainsAny = function _arrayContainsAny(
							toSearch,
							toFind
						) {
						...
						_parseVars = function _parseVars(params, type, parent) {
							...
							if (type) {
								...
								vars.immediateRender = _isNotFalse(
									irVars.immediateRender
								);
						...
						_hasNoPausedAncestors = function _hasNoPausedAncestors(
							...
							return (
								!animation ||
								(animation._ts &&
									_hasNoPausedAncestors(animation.parent))
							);
						},
						_elapsedCycleDuration = function _elapsedCycleDuration(
							...
							return animation._repeat
								? _animationCycle(
										animation._tTime,
										(animation =
											animation.duration() + animation._rDelay)
								  ) * animation
								: 0;
						},
						// feed in the totalTime and cycleDuration and it'll return the cycle (iteration minus 1) and if the playhead is exactly at the very END, it will NOT bump up to the next cycle.
						_animationCycle = function _animationCycle(
							tTime,
							cycleDuration
						) {
						...
						_setEnd = function _setEnd(animation) {
							return (animation._end = _round(
								animation._start +
									(animation._tDur /
										Math.abs(
											animation._ts || animation._rts || _tinyNum
										) || 0)
							));
						},
						/*
...
*/
						...
						_renderZeroDurationTween = function _renderZeroDurationTween(
							...
							if (
								...
								tween._zTime =
									totalTime || (suppressEvents ? _tinyNum : 0); // when the playhead arrives at EXACTLY time 0 (right on top) of a zero-duration tween, we need to discern if events are suppressed so that when the playhead moves again (next time), it'll trigger the callback. If events are NOT suppressed, obviously the callback would be triggered in this render. Basically, the callback should fire either when the playhead ARRIVES or LEAVES this exact spot, not both. Imagine doing a timeline.seek(0) and there's a callback that sits at 0. Since events are suppressed on that seek() by default, nothing will fire, but when the playhead moves off of that position, the callback should fire. This behavior is what people intuitively expect.
								...
								if (!distances) {
									...
									if (!wrapAt) {
										...
										while (
											max <
												(max =
													a[wrapAt++].getBoundingClientRect()
														.left) &&
											wrapAt < l
										) {}
						...
						_roundModifier = function _roundModifier(v) {
							...
							return function (raw) {
								return (
									Math.floor(Math.round(parseFloat(raw) / v) * v * p) /
										p +
						...
						snap = function snap(snapTo, value) {
							...
							return _conditionalReturn(
								...
								!isArray
									...
									? function (raw) {
											is2D = snapTo(raw);
											return Math.abs(is2D - raw) <= radius
												? is2D
												: raw;
									  }
									: function (raw) {
						...
						wrapYoyo = function wrapYoyo(min, max, value) {
							...
							return _isArray(min)
								...
								: _conditionalReturn(value, function (value) {
										value =
											(total + ((value - min) % total)) % total || 0;
										return (
											min + (value > range ? total - value : value)
										);
								  });
						},
						...
						mapRange = function mapRange(
							...
							return _conditionalReturn(value, function (value) {
								return (
									outMin +
									(((value - inMin) / inRange) * outRange || 0)
								);
							});
						},
						...
						splitColor = function splitColor(v, toHSL, forceAlpha) {
							...
							if (!a) {
								...
								if (_colorLookup[v]) {
									...
									a =
										v.match(_strictNumExp) ||
										_colorLookup.transparent;
								}
						...
						_formatColors = function _formatColors(
							s,
							toHSL,
							orderMatchData
						) {
						...
						_ticker = (function () {
							var _getTime = Date.now,
								...
								_tick = function _tick(v) {
									...
									if (overlap > 0 || manual) {
										...
										_nextTime +=
											overlap +
											(overlap >= _gap ? 0.004 : _gap - overlap);
										dispatch = 1;
									}
							...
							_self = {
								...
								sleep: function sleep() {
									(_raf ? _win.cancelAnimationFrame : clearTimeout)(
										_id
									);
									_tickerActive = 0;
									_req = _emptyFunc;
								},
								lagSmoothing: function lagSmoothing(
									threshold,
									adjustedLag
								) {
									_lagThreshold = threshold || 1 / _tinyNum; //zero should be interpreted as basically unlimited

									_adjustedLag = Math.min(
										adjustedLag,
										_lagThreshold,
										0
									);
						...
						_insertEase = function _insertEase(
							...
							_forEachName(names, function (name) {
								...
								_easeMap[(lowercaseName = name.toLowerCase())] =
									easeOut;
					...
					_easeMap.Linear.easeNone = _easeMap.none =
						_easeMap.Linear.easeIn;
						...
						_proto.paused = function paused(value) {
							...
							if (this._ps !== value) {
								...
								if (value) {
									this._pTime =
										this._tTime ||
										Math.max(-this._delay, this.rawTime()); // if the pause occurs during the delay phase, make sure that's factored in when resuming.
        ...
        */
								...
								this._start = _round(
									_ticker.time -
										(this._ts > 0
											...
											: (this.totalDuration() - _totalTime2) /
											  -this._ts)
								);
							}
							...
							while (child) {
								if (child instanceof Tween) {
									if (
										...
										(!onlyActive ||
											child.isActive(onlyActive === "started"))
									) {
										a.push(child);
									}
								} else if (
									(children = child.getTweensOf(
										parsedTargets,
										onlyActive
									)).length
								) {
							...
							function _addComplexStringPropTween(
								...
								while ((result = _complexStringNumExp.exec(end))) {
									...
									if (endNum !== startNums[matchIndex++]) {
										startNum =
											parseFloat(startNums[matchIndex - 1]) || 0; //these nested PropTweens are handled in a special way - we'll never actually call a render or setter method on them. We'll just loop through them in the parent complex string PropTween's render method.
						...
						_checkPlugin = function _checkPlugin(
							...
							if (
								...
								if (tween !== _quickTween) {
									ptLookup =
										tween._ptLookup[tween._targets.indexOf(target)]; //note: we can't use tween._ptLookup[index] because for staggered tweens, the index from the fullTargets array won't match what it is in each individual tween that spawns from the stagger.
					...
					var Tween = /*#__PURE__*/ (function (_Animation2) {
						...
						_proto3.render = function render(
							...
							if (!dur) {
								...
								this.ratio = ratio = (yoyoEase || this._ease)(
									time / dur
								);
					...
					var _setterPlain = function _setterPlain(
							target,
							property,
							value
						) {
						...
						_sortPropTweensByPriority =
							function _sortPropTweensByPriority(parent) {
								var pt = parent._pt,
									next,
									pt2,
									first,
					...
					var _gsap = {
						...
						isTweening: function isTweening(targets) {
							return (
								_globalTimeline.getTweensOf(targets, true).length > 0
							);
						},
  ...
  \************************************/
				...
				/***/ function (module, __webpack_exports__, __webpack_require__) {
					...
					/* harmony reexport (safe) */ __webpack_require__.d(
						...
						"TweenLite",
						function () {
							return _gsap_core_js__WEBPACK_IMPORTED_MODULE_0__[
								"TweenLite"
							];
						}
					);
					...
					/* harmony reexport (safe) */ __webpack_require__.d(
						...
						"CSSPlugin",
						function () {
							return _CSSPlugin_js__WEBPACK_IMPORTED_MODULE_1__[
								"CSSPlugin"
							];
						}
					);
										...
										for (
											...
											if (
												...
												for (i = 0; i < delegateCount; i++) {
													...
													if (
														matchedSelectors[sel] === undefined
													) {
									...
									this.body
										...
										.on("mousemove", function (e) {
											_this.pos = {
												x: _this.stick
													? _this.stick.x -
													  (_this.stick.x - e.clientX) * 0.15
													: e.clientX,
												y: _this.stick
													? _this.stick.y -
													  (_this.stick.y - e.clientY) * 0.15
													: e.clientY,
											};
											_this.update();
										})
  ...
  \*************************/
				...
				/***/ function (module, __webpack_exports__, __webpack_require__) {
					...
					var cursor = new cursor__WEBPACK_IMPORTED_MODULE_1__[
						"default"
					](); // Init magnetic
						...
						_createClass(Magnetic, [
							{
								...
								value: function bind() {
									...
									this.el.on("mouseenter", function () {
										_this.y =
											_this.el.offset().top - window.pageYOffset;
										_this.x =
											_this.el.offset().left - window.pageXOffset;
										_this.width = _this.el.outerWidth();
										_this.height = _this.el.outerHeight();
									});
									this.el.on("mousemove", function (e) {
										var y =
											(e.clientY - _this.y - _this.height / 2) *
											_this.options.y;
										var x =
											(e.clientX - _this.x - _this.width / 2) *
											_this.options.x;
